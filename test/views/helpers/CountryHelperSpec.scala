...
class CountryHelperSpec extends UnitSpec with Injector {
  ...
  "CountryHelper getShortNameForTransportCountry" should {
    "get the short name for a country when it is available" in {
      countryHelper.getShortNameForTransportCountry("US") mustBe "The United States of America"
    }

    "pass the name directly when a name is provided" in {
      countryHelper.getShortNameForTransportCountry("The United States of America") mustBe "The United States of America"
    }

    "get the full name for a country when a short name is not available" in {
      countryHelper.getShortNameForTransportCountry("NC") mustBe "New Caledonia, Including Loyalty Islands (Lifou, Mare and Ouvea)"
    }
  }
}
