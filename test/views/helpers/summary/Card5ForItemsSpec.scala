  ...
  "Items card" when {
    "the declaration has items" should {
      ...
      "show the 'Packing details' section" in {
        ...
        checkMultiRowSection(rows.get(0), List("item-1-package-information-1-number"), s"$keyId.number", "10", call, keyId)
        checkMultiRowSection(rows.get(1), List("item-1-package-information-1-type"), s"$keyId.type", "Parcel (PC)")
