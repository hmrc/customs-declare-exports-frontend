...
class ErrorsReportedHelperSpec extends UnitViewSpec with Injector with MockitoSugar with ExportsDeclarationBuilder with ExportsItemBuilder {
  ...
  when(countryHelper.getShortNameForTransportCountry(meq("GB"))(any())).thenReturn("United Kingdom")
  when(countryHelper.getShortNameForTransportCountry(meq("ZA"))(any())).thenReturn("South Africa")
