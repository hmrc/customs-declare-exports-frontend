...
import views.helpers.PointerRecordSpec.{
  ...
  "PointerPatterns" can {
    "expand single parent pointers into a sequence of their child pointers, so" should {
      s"expand '$additionalActorsPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq(s"${additionalActorsPointer.pattern}.eori", s"${additionalActorsPointer.pattern}.type"))
      }
      ...
      s"expand '$consignorDetailsPointer' into its child element" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq(s"${consignorDetailsPointer.pattern}.eori"))
      }
      ...
      s"expand '$declarationHoldersPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(
          Seq(s"${declarationHoldersPointer.pattern}.eori", s"${declarationHoldersPointer.pattern}.authorisationTypeCode")
        )
      }
      ...
      s"expand '$representativeDetailsPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(
          Seq(s"${representativeDetailsPointer.pattern}.details.eori", s"${representativeDetailsPointer.pattern}.statusCode")
        )
      }
      ...
      s"expand '$previousDocumentsPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq(s"$base.documentReference", s"$base.documentType"))
      }
      ...
      s"expand '$additionalInformationPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq(s"$base.code", s"$base.description"))
      }
      ...
      s"expand '$additionalFiscalReferencesPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq("declaration.items.$.additionalFiscalReferences.$.roleCode"))
      }
      ...
      s"expand '$cusCodePointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq(s"${cusCodePointer.pattern}.cusCode"))
      }

      s"expand '$itemsPointer' into its child elements" when {
        val itemsPointerBase = itemsPointer.pattern

        def checkExpandedPointers(expandedPointers: Seq[Pointer]) =
          expandedPointers.map(_.pattern) must equal(
            Seq(
        ...
        "original dec has no items" in {
          checkExpandedPointers(PointerPatterns.expandPointer(itemsPointer, emptyeDec, populatedDec))
        }
        "amended dec has no items" in {
          checkExpandedPointers(PointerPatterns.expandPointer(itemsPointer, populatedDec, emptyeDec))
        }
      }
      ...
      s"expand '$containerPointer' into its child elements" in {
        ...
        expandedPointers.map(_.pattern) must equal(Seq(s"${containerPointer.pattern}.id", s"${containerPointer.pattern}.seals.ids"))
      }
    }
  }
