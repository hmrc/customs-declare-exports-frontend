...
class EoriOrAddressSectionViewSpec extends UnitViewSpec with ExportsTestData {
  ...
  val country = "Country"
  val address = Seq(street, city, postCode, country)
  ...
  "Eori or address section" should {
    ...
    "display eori row without value if eori and address is empty and eori is default row" in {
      ...
      val view = eori_or_address_section(pageId, None, eoriLabel, Seq.empty, addressLabel)(messages, journeyRequest())
    ...
    "display address row without value if eori and address is empty and eori is not default row" in {
      ...
      val view = eori_or_address_section(pageId, None, eoriLabel, Seq.empty, addressLabel, false)(messages, journeyRequest())
    ...
    "display eori when defined" in {
      ...
      val view = eori_or_address_section(pageId, Some(eori), eoriLabel, Seq.empty, addressLabel)(messages, journeyRequest())
    ...
    "display address when defined" in {
      ...
      val view = eori_or_address_section(pageId, None, eoriLabel, address, addressLabel)(messages, journeyRequest())

      view.getElementById(s"$pageId-address-label").text() mustBe messages(addressLabel)
      view.getElementById(s"$pageId-address-0").text() mustBe street
      view.getElementById(s"$pageId-address-1").text() mustBe city
      view.getElementById(s"$pageId-address-2").text() mustBe postCode
      view.getElementById(s"$pageId-address-3").text() mustBe country
    }

    "display both eori and address when defined" in {

      val view = eori_or_address_section(pageId, Some(eori), eoriLabel, address, addressLabel)(messages, journeyRequest())
