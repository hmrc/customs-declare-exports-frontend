...
class PartiesSectionHoldersViewSpec extends UnitViewSpec with ExportsTestData with Injector {
  ...
  "Holders (authorised) parties section" should {
    ...
    "display holders if exists" in {
      ...
      row1ChangeLink.text() mustBe messages("site.change") + messages("declaration.summary.parties.holders.change")
      ...
      row2ChangeLink.text() mustBe messages("site.change") + messages("declaration.summary.parties.holders.change")
    }
  }
}
