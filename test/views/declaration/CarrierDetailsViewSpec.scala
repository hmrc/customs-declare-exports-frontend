...
import forms.declaration.{Address, CarrierDetails, EntityDetails}
  ...
  class CarrierDetailsViewSpec extends ViewSpec with CarrierDetailsMessages with CommonMessages {
    ...
    "Carrier Details View on empty page" should {
      ...
      "display empty input with label for EORI" in {
        ...
        getElementByCss(view, "label.form-label>span.form-hint").text() must be(messages(carrierEoriHint))
    ...
    "Carrier Details View with invalid input" should {
      ...
      "display error when EORI is provided, but is incorrect" in {
        ...
        val view = createView(
          CarrierDetails
            .form()
            .fillAndValidate(CarrierDetails(EntityDetails(Some(TestHelper.createRandomString(18)), None)))
        )
    ...
    "Carrier Details View when filled" should {
      ...
      "display data in both EORI and Business address inputs" in {
        ...
        val form = CarrierDetails
          .form()
          .fill(
            CarrierDetails(EntityDetails(Some("1234"), Some(Address("test", "test1", "test2", "test3", "test4"))))
          )
        val view = createView(form)
