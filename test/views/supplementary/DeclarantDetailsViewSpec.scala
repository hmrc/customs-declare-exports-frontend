...
@ViewTest
class DeclarantDetailsViewSpec extends ViewSpec {

  private val form: Form[DeclarantDetails] = DeclarantDetails.form()

  private val prefix = s"${basePrefix}declarant."
  ...
  "Declarant Details View" should {
    ...
    "have proper messages for labels" in {
      ...
      assertMessage(title.withPrefix, "3/17 - 3/18 Add declarant")
      assertMessage(title.withHint, "TODO")
  ...
  "Declarant Details View with invalid input" should {
    ...
    /*
     * so this is due to composition, when we put the field, we need to specify is with correct path
     * e.g: details.eori is part of details form which is put into bigger form, so to produce error
     * for it we need to specify full path
     */
    "display error when both EORI and business details are not provided" in {

      val view = createView(DeclarantDetails.form()
