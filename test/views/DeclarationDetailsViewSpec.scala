...
import models.declaration.submissions.SubmissionStatus.{ACCEPTED, RECEIVED, REJECTED, UNKNOWN}
...
class DeclarationDetailsViewSpec extends UnitViewSpec with Injector {
  ...
  private val notifications = List(
    Notification("id", "mrn", now, RECEIVED, Seq.empty),
    Notification("id", "mrn", now, UNKNOWN, Seq.empty),
    Notification("id", "mrn", now, ACCEPTED, Seq.empty),
    Notification("id", "mrn", now, REJECTED, Seq.empty)
  )
