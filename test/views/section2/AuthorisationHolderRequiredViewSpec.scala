...
@ViewTest
class AuthorisationHolderRequiredViewSpec extends UnitViewSpec with ExportsTestHelper with CommonMessages with Stubs with Injector {

  private val form = YesNoAnswer.form()
  private val authorisationHolderRequiredPage = instanceOf[authorisation_holder_required]

  private val prefix = "declaration.authorisationHolderRequired"

  private def view(implicit request: JourneyRequest[_]): Document =
    authorisationHolderRequiredPage(form)
    ...
    "have back link" should {
      ...
      onJourney(STANDARD, SUPPLEMENTARY) { implicit request =>
        "display 'Back' button that links to the 'Authorisation Choice' page" in {
          ...
          backButton must haveHref(AuthorisationProcedureCodeChoiceController.displayPage)
        }
      }
