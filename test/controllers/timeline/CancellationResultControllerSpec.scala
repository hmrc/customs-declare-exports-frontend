  ...
  "CancellationResultController.displayHoldingPage" should {
    ...
    "return the expected page" when {
      ...
      "the request does not include a 'js' query parameter" in {
        ...
        val holdingUrl = CancellationResultController.displayHoldingPage.url
      ...
      "the request's query parameter 'js' is equal to 'disabled'" in {
        ...
        jsDisabledCaptor.getValue.asInstanceOf[String] mustBe CancellationResultController.displayResultPage.url
    ...
    "return 303(SEE_OTHER) status code" when {
      "the request's query parameter 'js' is equal to 'disabled'" in {
        ...
        redirectLocation(result) mustBe Some(CancellationResultController.displayResultPage.url)
      }
    }
