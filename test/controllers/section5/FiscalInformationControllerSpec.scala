...
import controllers.section5.routes.{AdditionalFiscalReferenceAddController, AdditionalFiscalReferencesController, CommodityDetailsController}
...
import views.html.section5.fiscalInformation.fiscal_information
  ...
  "Fiscal Information controller" should {
    ...
    "return 303 (SEE_OTHER)" when {
      ...
      "user answer yes with no fiscal references in cache yet" in {
        ...
        await(result) mustBe aRedirectToTheNextPage
        thePageNavigatedTo mustBe AdditionalFiscalReferenceAddController.displayPage(item.id)
        verifyAudit()
      }
