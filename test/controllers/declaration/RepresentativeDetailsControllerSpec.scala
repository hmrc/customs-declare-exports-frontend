...
class RepresentativeDetailsControllerSpec
  ...
  "Representative Address Controller on POST" should {
    ...
    "accept form with status and EORI if on supplementary journey" in {
      withNewCaching(supplementaryModel)
      val result = route(app, postRequest(uri, correctRepresentativeDetailsEORIOnlyJSON, sessionId = supplementaryModel.sessionId)).get
    ...
    "accept form with status and EORI if on standard journey" in {
      ...
      val result = route(app, postRequest(uri, correctRepresentativeDetailsEORIOnlyJSON, sessionId = standardModel.sessionId)).get
    ...
    "accept form with status and address if on supplementary journey" in {
      withNewCaching(supplementaryModel)
      val result = route(app, postRequest(uri, correctRepresentativeDetailsAddressOnlyJSON, sessionId = supplementaryModel.sessionId)).get
    ...
    "accept form with status and address only if on standard journey" in {
      withNewCaching(standardModel)
      val result = route(app, postRequest(uri, correctRepresentativeDetailsAddressOnlyJSON, sessionId = standardModel.sessionId)).get
    ...
    "return 303 code" when {
      "data is correct" in {
        withNewCaching(supplementaryModel)
        val result = route(app, postRequest(uri, correctRepresentativeDetailsJSON, sessionId = supplementaryModel.sessionId)).get
    ...
    "redirect to Consignee Details page if on standard journey" in {
      ...
      val result = route(app, postRequest(uri, correctRepresentativeDetailsJSON, sessionId = standardModel.sessionId)).get
