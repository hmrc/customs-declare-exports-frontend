...
class NatureOfTransactionControllerSpec extends CustomExportsBaseSpec with NatureOfTransactionMessages {
  ...
  "Nature Of Transaction Controller on POST" should {
    ...
    "validate request and redirect - empty value" in {
      ...
      val result = route(app, postRequest(uri, emptyNatureOfTransactionJSON, sessionId = supplementaryModel.sessionId)).get
    ...
    "validate request and redirect - incorrect values" in {
      ...
      val result = route(app, postRequest(uri, incorrectNatureOfTransactionJSON, sessionId = supplementaryModel.sessionId)).get
    ...
    "validate request and redirect - correct values" in {
      ...
      val result = route(app, postRequest(uri, correctNatureOfTransactionJSON, sessionId = supplementaryModel.sessionId)).get
