...
import forms.journey.JourneySelection.{form, nonStandardJourneys, NonStandardDeclarationType}
  ...
  "StandardOrOtherJourneyController.submitChoice" should {
    ...
    "return 303 (SEE_OTHER)" when {
      ...
      "user selects the 'Other declaration type' radio" in {
        ...
        status(result) must be(SEE_OTHER)
        redirectLocation(result) must be(Some(OtherJourneyController.displayPage.url))
        verifyTheCacheIsUnchanged()
      }
