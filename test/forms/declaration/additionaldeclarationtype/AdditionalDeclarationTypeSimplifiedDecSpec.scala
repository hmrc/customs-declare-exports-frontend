...
class AdditionalDeclarationTypeSimplifiedDecSpec extends WordSpec with MustMatchers {

  private val validTypes = Seq(AdditionalDeclarationType.SIMPLIFIED_PRE_LODGED, AdditionalDeclarationType.SIMPLIFIED_FRONTIER)

  "AdditionalDeclarationType mapping user for binding data" should {

    "return form with errors" when {
      "provided with empty input" in {
        val form = AdditionalDeclarationTypeSimplifiedDec.form().bind(Map[String, String]())
      ...
      "provided with a value not defined in AllowedAdditionalDeclarationTypes" in {
        val form = AdditionalDeclarationTypeSimplifiedDec.form().bind(Map[String, String]("additionalDeclarationType" -> "#"))
      ...
      for (additionalType: AdditionalDeclarationType <- AdditionalDeclarationType.values.filterNot(validTypes.contains)) {
        s"provided with an invalid AdditionalDeclarationType $additionalType" in {
          val form = AdditionalDeclarationTypeSimplifiedDec.form().bind(Map[String, String]("additionalDeclarationType" -> additionalType.toString))
    ...
    "return form without errors" when {
      for (additionalType: AdditionalDeclarationType <- validTypes) {
        s"provided with valid input $additionalType" in {
          val form = AdditionalDeclarationTypeSimplifiedDec
            .form()
