...
trait CustomExportsBaseSpec
    ...
    with MockConnectors with BeforeAndAfterEach with ExportsDeclarationBuilder with MockExportsDeclarationService {
  ...
  override def withNewCaching(dataToReturn: ExportsDeclaration) {
    ...
    when(
      mockExportsCacheService
        .update(ArgumentMatchers.eq(dataToReturn.sessionId), any[ExportsDeclaration])
