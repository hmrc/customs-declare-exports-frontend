...
object Mrn {
  ...
  implicit val format: Format[Mrn] = Format[Mrn](Reads.StringReads.map(Mrn.apply), Writes[Mrn](mrn => JsString(mrn.value)))

  private val validRegex = "\\d{2}[a-zA-Z]{2}[a-zA-Z0-9]{14}"

  val isValid: String => Boolean = (input: String) => input.matches(validRegex)

  def mapping(prefix: String): Mapping[Mrn] =
    text()
