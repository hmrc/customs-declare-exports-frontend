...
object DepartureTransport extends DeclarationPage {
  ...
  def form(transportCodes: TransportCodeCollection): Form[DepartureTransport] = Form(mappingFor(transportCodes))

  private def mappingFor(transportCodes: TransportCodeCollection): Mapping[DepartureTransport] =
    mapping(
  ...
  private def model2Form(transportCodes: TransportCodeCollection): DepartureTransport => Option[
  ...
  private def mappingForRadioDepartureType(transportCodes: TransportCodeCollection): Mapping[Option[String]] = {
    val isV3WhenPC0019 = transportCodes == transportCodesForV3WhenPC0019

    optional(text.verifying(s"$prefix.error.incorrect", isContainedIn(transportCodes.transportCodes.map(_.value))))
