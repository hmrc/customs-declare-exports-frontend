...
import utils.validators.forms.FieldValidator.{isValidTraderReference, nonEmpty}
...
object TraderReference extends DeclarationPage {
  ...
  def form2data(traderReference: String): TraderReference = TraderReference(traderReference.trim.toUpperCase)

  def form: Form[TraderReference] =
    Form(
      mapping(
        "traderReference" -> text()
          .verifying("declaration.traderReference.error.empty", nonEmpty)
          .verifying("declaration.traderReference.error.invalid", isValidTraderReference)
