...
class PreviousDocumentsHelper @Inject()(
  ...
  def helpForDocumentReference(implicit messages: Messages, request: JourneyRequest[_]): Html =
    versionSelection match {
      case 1 => new Html(List(paragraph("v1.documentReference.body"), hint("v1.documentReference.hint")))
      case 2 => new Html(List(paragraph("v2.documentReference.body"), hint("v2.documentReference.hint")))
