...
case class Page(index: Int = Page.DEFAULT_INDEX, size: Int = Page.DEFAULT_MAX_SIZE)

object Page {
  val DEFAULT_INDEX = 1
  val DEFAULT_MAX_SIZE = 25
  ...
  implicit val bindable: QueryStringBindable[Page] =
    new QueryStringBindable[Page] {
      ...
      override def bind(key: String, params: Map[String, Seq[String]]): Option[Either[String, Page]] = {
        ...
        val size = params.get(queryParamSize(key)).flatMap(_.headOption).map(_.toInt).getOrElse(DEFAULT_MAX_SIZE)
