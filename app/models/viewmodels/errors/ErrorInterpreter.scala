...
trait ErrorInterpreter {
  ...
  def fieldsTableHeader(implicit messages: Messages) = Html(
  ...
  def errorTitle(error: ErrorInstance)(implicit messages: Messages) = Html(
    s"""<div class="govuk-summary-card__title-wrapper">
       |<h3 class="govuk-summary-card__title">Error ${error.seqNbr} - ${messages(s"dmsError.${error.errorCode}.title")} (${error.errorCode}) </h3></div>""".stripMargin)

  def errorChangeAction(call: Call, validationCode: String, pointer: Option[Pointer], decId: String, isAmendment: Boolean)
    (implicit messages: Messages, link: link): Html = {
...
object DefaultInterpreter extends ErrorInterpreter {
  def generateHtmlFor(error: ErrorInstance)(implicit messages: Messages, codeListConnector: CodeListConnector, link: link): Option[Html] = {
    ...
    val specificErrorContent = error.errorCode match {
      case "CDS10001" => CDS10001Interpreter.generateHtmlFor(error)
      case "CDS12062" => CDS12062Interpreter.generateHtmlFor(error)
      case "CDS12119" => CDS12119Interpreter.generateHtmlFor(error)
      case "CDS13000" => CDS13000Interpreter.generateHtmlFor(error)
      ...
      HtmlContent(
        HtmlFormat.fill(
          immutable.Seq(
            ...
            //Html(s"<p>${messages("rejected.notification.description.format", error.errorCode)}</p>"),
...
object CDS10001Interpreter extends ErrorInterpreter {
  def generateHtmlFor(error: ErrorInstance)(implicit messages: Messages, codeListConnector: CodeListConnector, link: link): Option[Html] =
    error.fieldsInvolved.map(_.pointer.toString).headOption match {
      case Some("42A.67A.1.55B") => CDS10001InlandModeOfTransportInterpreter.generateHtmlFor(error)
      case _                     => defaultContent()
    }

  private def defaultContent(): Option[Html] =
    Some(Html("CDS10001Interpreter Default"))
}

object CDS10001InlandModeOfTransportInterpreter extends ErrorInterpreter {
  def generateHtmlFor(error: ErrorInstance)(implicit messages: Messages, codeListConnector: CodeListConnector, link: link): Option[Html] =
    None
}
