...
object CDS12119Interpreter extends ErrorInterpreter {
  def generateHtmlFor(error: ErrorInstance)(implicit messages: Messages, codeListConnector: CodeListConnector, link: link): Option[Html] = {
    ...
    val changeLink = errorChangeAction(ProcedureCodesController.displayPage(error.sourceDec.items(itemIdx).id), error.errorCode, Some(pointer), error.sourceDec.id, false)
  ...
  def tableHeader(changeLink: Html)(implicit messages: Messages) = Html(
