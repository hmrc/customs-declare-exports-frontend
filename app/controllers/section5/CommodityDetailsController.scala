...
import controllers.section5.routes.{CommodityMeasureController, PackageInformationSummaryController, UNDangerousGoodsCodeController}
...
import views.html.section5.commodity_details
...
class CommodityDetailsController @Inject() (
  ...
  private def nextPage(itemId: String)(implicit request: JourneyRequest[AnyContent]): Result = {
    ...
    (request.declarationType, request.cacheModel.isNotExs) match {
      ...
      case (CLEARANCE, true) => navigator.continueTo(PackageInformationSummaryController.displayPage(itemId))
