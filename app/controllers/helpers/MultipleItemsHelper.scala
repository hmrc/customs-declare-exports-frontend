...
object MultipleItemsHelper {
   ...
   */
  def add[A](form: Form[A], cachedData: Seq[A], limit: Int, fieldId: String = "", messageKey: String)(
    implicit handler: MultipleItemsDuplicateHandler[A]
  ): Either[Form[A], Seq[A]] =
    form.value match {
  ...
  private def prepareData[A](form: Form[A], document: A, cachedData: Seq[A], limit: Int, fieldId: String, messageKey: String)(
    implicit handler: MultipleItemsDuplicateHandler[A]
  ): Either[Form[A], Seq[A]] =
    handler.duplication(document, cachedData, fieldId, messageKey) ++ limitOfElems(limit, cachedData, fieldId) match {
   ...
   */
  def saveAndContinue[A](form: Form[A], cachedData: Seq[A], isMandatory: Boolean, limit: Int, fieldId: String = "", messageKey: String)(
    implicit handler: MultipleItemsDuplicateHandler[A]
  ): Either[Form[A], Seq[A]] =
