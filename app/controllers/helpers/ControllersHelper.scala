...
object ControllersHelper {
  ...
  def trimRequestBody(request: Request[_]): Map[String, Seq[String]] = transformBodyIntoFormUrlEncoded(request).map {
    case (key, values) => (key, values.map(_.trim))
  }

  private def transformBodyIntoFormUrlEncoded(request: Request[_]): Map[String, Seq[String]] = {
    (request.body match {
