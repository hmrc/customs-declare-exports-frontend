...
class CancelDeclarationController @Inject()(
  ...
  private def createCancellationMetadata(form:CancelDeclarationForm):MetaData =
    MetaData(wcoDataModelVersionCode = form.wcoDataModelVersionCode,
    wcoTypeName = form.wcoTypeName,
      ...
      declaration = Some(Declaration(functionCode = Some(CANCEL_FUNCTION_CODE),
        ...
        additionalInformations = Seq(uk.gov.hmrc.wco.dec.AdditionalInformation(form.additionalInformation.statementCode,
          Some(form.additionalInformation.statementDescription),statementTypeCode = Some(form.additionalInformation.statementTypeCode),
