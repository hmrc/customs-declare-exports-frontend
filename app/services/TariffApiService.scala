...
@Singleton
class TariffApiService @Inject()(tariffApiConnector: TariffApiConnector)(implicit ec: ExecutionContext) extends Logging {

  def retrieveCommodityInfo(commodityCode: String): Future[Option[CommodityInfo]] =
  ...
  private def extractIncludedObj(commodityCode: String, json: JsValue): Option[CommodityInfo] =
    parseJsValue[JsArray](json, "included").flatMap(extractCommodityInfoFromIncludedObj(commodityCode, _))

  private def extractCommodityInfoFromIncludedObj(commodityCode: String, included: JsArray): Option[CommodityInfo] = {
  ...
  private def extractAllMeasureObjs(objs: Seq[JsValue]): Seq[JsValue] =
    objs.filter(parseJsValue[String](_, "type").exists(_ == "measure"))
  ...
  private def extractCommodityInfo(commodityCode: String, abbrTag: String): Option[CommodityInfo] =
    abbrTag match {
      case pattern(description, units) => Some(CommodityInfo(commodityCode, description.trim, units.trim))
      case _ => None
    }
  ...
  private def extractIdOfDutyExpressionObj(objs: Seq[JsValue]): Option[String] =
    extractAllMeasureObjs(objs)
      .filter(measureObjsWithId109Only)
      .map(extractIdOfDutyExpressionObjFromMeasureObjWithId109)
      .headOption
      .flatten

  private def measureObjsWithId109Only(obj: JsValue): Boolean =
